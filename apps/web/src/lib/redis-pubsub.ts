...
export function createRedisClient(): Redis {
  ...
  throw new Error(
    'Upstash Redis does not support Pub/Sub. Please use standard Redis for real-time features.'
  )
}
...
export async function closeRedisConnections() {
  ...
  const closePromises = Array.from(activeSubscribers.values()).map(client =>
    client.quit().catch(err => logger.error(err, { context: 'Error closing subscriber' }))
  )
