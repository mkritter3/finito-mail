#!/usr/bin/env tsx
/**
 * Generate RLS Migration File Content
 * This script generates the SQL migration content for enabling RLS policies
 * 
 * Usage: npm run rls:phase2:generate-migration
 */
...
async function generateMigration() {
  console.log('🚀 Generating RLS Migration File')
  console.log('================================\n')

  const timestamp = new Date().toISOString().replace(/[-:T]/g, '').split('.')[0]
  const filename = `${timestamp}_enable_rls_policies.sql`
  const filepath = path.join(process.cwd(), 'supabase', 'migrations', filename)

  try {
    // Ensure migrations directory exists
    await fs.mkdir(path.join(process.cwd(), 'supabase', 'migrations'), { recursive: true })

    // Write migration file
    await fs.writeFile(filepath, MIGRATION_CONTENT)

    console.log(`✅ Generated migration file: supabase/migrations/${filename}`)
    console.log('\n📋 Next steps:')
    console.log('1. Review the migration file')
    console.log('2. Run: supabase db push')
    console.log('3. Or apply manually in Supabase SQL Editor')
    console.log('\n⚠️  WARNING: This will enable RLS on all tables!')
    console.log('Make sure you have completed Phase 1 baseline first.')
